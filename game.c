...
STATUS game_add_space(Game *game, Space *space)
{
  ...
  //Añade el espacio space a los espacios
  game->spaces[i] = space;
  ...
  for (i = 0; i < MAX_SPACES && game->spaces[i] != NULL; i++)
  {
    ...
    if (current_id == space_id)
    {
      ...
      if (current_id != NO_ID)
      {
        ...
        {
          object_set_id(game->obj, current_id);
        }
...
void game_command_take(Game *game)
{
  if(player_get_object(game->bob) != NULL)
    return;
  if (object_get_id(game->obj)==player_get_id(game->bob))
  {
    player_set_object(game->bob,game->obj);
  }
